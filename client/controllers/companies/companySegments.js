...
Template.companySegments.onCreated(function() { });
Template.companySegments.rendered = function(){
  checkSession();
  settingLanguage();
  $('.footable').footable();
  $('.footable2').footable();
...
Template.companySegments.events({
    'click .newPlan'(){
      $('#startDate').val("");
      $('#endDate').val("");
      $('#newPlan').modal();
...
Template.companySegments.helpers({
  plansLive (){
    return Plans_Live.find({ 'codeCompany': Session.get("COMPANY_CODE"), 'screensID': Session.get("SCREEN_ID") });
  },
  plansAuthorization (){
    var plans = Plans_Authorization.find({ 'codeCompany': Session.get("COMPANY_CODE"), 'screensID': Session.get("SCREEN_ID") });
