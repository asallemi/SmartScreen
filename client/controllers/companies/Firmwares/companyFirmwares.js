...
Template.companyFirmwares.onCreated(function () {
});
Template.companyFirmwares.rendered = function(){
  checkSession();
  settingLanguage();
  $('.footable').footable();
  $('.footable2').footable();
};
Template.companyFirmwares.events({
  'click .upload'() {
    $('#upload').modal();
    ...
    Meteor.call('cleanPackages');
    //Meteor.call('newPackage', "/home/akrem/packages", Session.get("UserLogged")._id, Session.get("COMPANY_CODE"));
    Meteor.call('newPackage', "/home/akrem/packages", Session.get("UserLogged")._id, Session.get("COMPANY_CODE"), function(error, result){
      if (result == 0) {
...
Template.companyFirmwares.helpers({
  firmwaresLive (){
    return Firmwares_Live.find({ 'codeCompany': Session.get("COMPANY_CODE") });
  },
  firmwaresAuthorization (){
    var firmwares = Firmwares_Authorization.find({ 'codeCompany': Session.get("COMPANY_CODE") });
