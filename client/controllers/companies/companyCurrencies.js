...
// A function which returns true if the company had a currency pivot
...
Template.companyCurrencies.rendered = function(){
  checkSession();
  settingLanguage();
  // Initialize fooTable
  $('.footable').footable();
...
Template.companyCurrencies.events({
  'click .newCurrency'() {
    // To empty all inputs
    $('#currencyName').val("");
    $('#currencyCode').val("");
  ...
  'click .authorizeAu'() {
    ...
    if (newCurrency.isPivot == true && verifyPivotCurrency(Session.get("COMPANY_CODE")) == true && newCurrency._id.indexOf("#D") < 0) {
...
Template.companyCurrencies.helpers({
  currenciesLive (){
    return Currencies_Live.find({ "codeCompany": Session.get("COMPANY_CODE") });
  },
  currenciesAuthorization (){
    var currencies = Currencies_Authorization.find({ "codeCompany": Session.get("COMPANY_CODE") });
  ...
  pivotCurrency (){
    return getPivotCurrency(Session.get("COMPANY_CODE"));
  },
  currencyDetails(){
