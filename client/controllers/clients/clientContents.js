...
Template.allClientContents.onCreated(function() {
...
Template.allClientContents.rendered = function(){
    checkSession();
    settingLanguage();
    // Initialize fooTable
    $('.footable').footable();
...
Template.allClientContents.events({
  'click .newContent'() {
    $('#upload').modal();
    Meteor.call('newContent', Session.get("URL"), Session.get("CLIENT_CODE"), Session.get("UserLogged").codeCompany, Session.get("UserLogged")._id);
  },
  'click .backButton'() {
    if(whoIsConnected() == "swallow-labs"){
      Router.go('companyClients');
    }else {
...
Template.allClientContents.helpers({
  contentsLive() {
    return Contents_Live.find({ "code": Session.get("CLIENT_CODE") });
  },
  contentsAuthorization(){
    var contents = Contents_Authorization.find({ "code": Session.get("CLIENT_CODE") });
